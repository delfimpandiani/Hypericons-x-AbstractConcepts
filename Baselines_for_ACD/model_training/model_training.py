...
def multi_class_classifier(data_prep_type, train_loader, val_loader, test_loader, base_model_name, model, experiment_name, epochs, augmented_data):
    ...
    writer.add_scalar('F1Score/Test', test_f1_score, epoch)
...
model_names = ["vgg16", "resnet50", "vit"]
for name in model_names:
    model = load_pretrained_model(name)
    model = multi_class_classifier(data_prep_type, train_loader, val_loader, test_loader, name, model, experiment_name, epochs, augmented_data)
    trainable_params = count_trainable_params(model)
    print(f"Trainable parameters for {name}: {trainable_params}")